...
export class DeleteModalComponent implements OnInit {
  ...
  async deleteDocument() {
    ...
    switch (this.collection) {
      case Utils.COLLECTION_STAFF: {
        await this.stRef.ref('staffs/' + this.docId).listAll()
          .toPromise()
          .then(stgItem => {
    ...
    this.dbRef.collection(Utils.MAIN).doc(Utils.MAIN).collection(this.collection).doc(this.docId)
      .delete()
      .then(() => {
        this.dialogRef.close();
        this.snackBar.open(this.collection.split(Utils.HYPHEN)[0].toUpperCase() + " Deleted Successfully", "", {
          duration: 6000
        });
      });
  }
